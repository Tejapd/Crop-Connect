...
def visitor_find_crop(request):
    ...
    if request.method == 'POST':
        ...
        file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Machine Learning', 'Crop_recommendation.csv')
...
@login_required(login_url='visitor_login')
@user_passes_test(is_visitor)
def visitor_disease_prediction(request):
    result_description = ''
    result_prevention = ''
    if request.method == 'POST':
        form = FindDiseaseForm(request.POST)
        if form.is_valid():
            crop_name = form.cleaned_data['crop_name'].strip()
            disease_name = form.cleaned_data['disease_name'].strip()

            file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Machine Learning', 'disease_info.csv')
            df = pd.read_csv(file_path, encoding='latin1')

            # Strip and lowercase the crop_name and disease_name columns for better matching
            df['crop_name'] = df['crop_name'].str.strip().str.lower()
            df['disease_name'] = df['disease_name'].str.strip().str.lower()

            # Debug print statements
            print(f"Searching for crop: '{crop_name}', disease: '{disease_name}'")

            disease_data = df[(df['crop_name'] == crop_name.lower()) & (df['disease_name'] == disease_name.lower())]

            if not disease_data.empty:
                result_description = disease_data.iloc[0]['disease_description']
                result_prevention = disease_data.iloc[0]['disease_prevention']
                print("Match found, returning disease info.")
            else:
                result_description = "No data found for the given crop and disease."
                result_prevention = ""
                print("No match found.")

    else:
        form = FindDiseaseForm()

    context = {
        'form': form,
        'result_description': result_description,
        'result_prevention': result_prevention,
    }
    return render(request, 'visitor/visitor_disease_prediction.html', context)

@login_required(login_url='visitor_login')
@user_passes_test(is_visitor)
def visitor_fertilizer_prediction(request):
    result = ''
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Machine Learning', 'Fertilizer_prediction.csv')
            df = pd.read_csv(file_path)
            ...
            soil_encoded = le_soil.transform([soil_type])[0] if soil_type in le_soil.classes_ else -1
            crop_encoded = le_crop.transform([crop_type])[0] if crop_type in le_crop.classes_ else -1

            # Convert input to lowercase for case-insensitive matching
            soil_type_lower = soil_type.lower()
            crop_type_lower = crop_type.lower()

            # Create mapping from lowercase to original classes
            soil_classes_lower = {cls.lower(): cls for cls in le_soil.classes_}
            crop_classes_lower = {cls.lower(): cls for cls in le_crop.classes_}

            if soil_type_lower in soil_classes_lower:
                soil_encoded = le_soil.transform([soil_classes_lower[soil_type_lower]])[0]
            else:
                soil_encoded = -1

            if crop_type_lower in crop_classes_lower:
                crop_encoded = le_crop.transform([crop_classes_lower[crop_type_lower]])[0]
            else:
                crop_encoded = -1

            if soil_encoded == -1 or crop_encoded == -1:
                result = "Invalid Soil Type or Crop Type entered. / No fertilizers found for the given input."
            else:
                from sklearn.ensemble import RandomForestClassifier
                from sklearn.model_selection import train_test_split
                ...
                if prediction[0]:
                    result = "The predicted fertilizer is " + str(prediction[0])
                else:
                    result = "No fertilizers found for the given input."

    else:
        form = FertilizerPredictionForm()

    return render(request, 'visitor/visitor_fertilizer_prediction.html', {'form': form, 'result': result})
